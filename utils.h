...
inline bool is_inarray(int *s,int sz, int v){
    for(int i = 0; i < sz; ++i)
        if(s[i] ==v)
            return true;
    return false;
...
void mtx_rand_init(gsl_matrix *x);
...
void mtx_norm_init(gsl_matrix *x);
...
inline void ul_gsl_matrix_add_row(gsl_matrix *m, gsl_vector *v, int r){
    gsl_vector_view rr = gsl_matrix_row(m,r);
    gsl_vector_add(&rr.vector,v);

}

inline void ul_gsl_matrix_div_vector(gsl_matrix *m, gsl_vector *v){
    for(int i = 0; i < m->size1; ++i)
    {
        gsl_vector_view rr = gsl_matrix_row(m,i);
        gsl_vector_div(&rr.vector,v);
    }
}
inline void ul_gsl_matrix_add_vector(gsl_matrix *m, gsl_vector *v){
    for(int i = 0; i < m->size1; ++i)
    {
        gsl_vector_view rr = gsl_matrix_row(m,i);
        gsl_vector_add(&rr.vector,v);
